...
// jiraConfigResponse wraps the persisted config with the resolved destination endpoint.
type jiraConfigResponse struct {
	db.JiraConfig
	Endpoint string `json:"endpoint,omitempty"`
}

// GET /api/v1/system/jiraConfig
func (h *Handler) GetJiraConfig(ctx *gin.Context) {
	cfg, err := h.svc.GetJiraConfig()
	if err != nil {
		OK(ctx, db.JiraConfig{})
		return
	}
	resp := jiraConfigResponse{JiraConfig: *cfg}
	if cfg.DestinationName != "" {
		if dest, err := h.destSvc.GetDestination(ctx.Request.Context(), cfg.DestinationName); err == nil && dest != nil {
			resp.Endpoint = dest.URL
		}
	}
	OK(ctx, resp)
}

// PUT /api/v1/system/jiraConfig
func (h *Handler) UpdateJiraConfig(ctx *gin.Context) {
	var req struct {
	...
	cfg, err := h.svc.UpdateJiraConfig(req.DestinationName, req.Enabled)
	if err != nil {
		FailCode(ctx, 500, errcode.Internal, fmt.Sprintf("failed to update jira config: %s", err))
		return
	}
...
// POST /api/v1/system/jiraConfig/test
func (h *Handler) TestJiraConnection(ctx *gin.Context) {
	result := h.svc.TestJiraConnection(ctx.Request.Context())
	OK(ctx, result)
}
...
// --- System Connectivity Check (individual, real-time, no persist) ---
