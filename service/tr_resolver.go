...
func (s *Service) GenerateTransportRequest(ctx context.Context, tenantID, deliveryRequestID uint, artifactOperationIDs []uint) (map[uint]*TransportRequest, map[uint]error, error) {
	...
	if dbErr := s.DB.WithContext(ctx).Model(&db.ArtifactTenantOperation{}).
		...
		s.L(ctx).Warnw("failed to set ops to TR_GENERATING", "component", "generate_tr", "error", dbErr)
	}
	...
	fatalf := func(err error) (map[uint]*TransportRequest, map[uint]error, error) {
		...
		if dbErr := s.DB.WithContext(ctx).Model(&db.ArtifactTenantOperation{}).
			...
			s.L(ctx).Warnw("failed to reset TR_GENERATING ops on fatal error", "component", "generate_tr", "error", dbErr)
		}
	...
	for range ops {
		...
		if r.err != nil {
			...
			if dbErr := s.DB.WithContext(ctx).Model(&db.ArtifactTenantOperation{}).
				...
				s.L(ctx).Warnw("failed to write TR_FAILED for op",
		...
		if dbErr := s.DB.WithContext(ctx).Model(&db.ArtifactTenantOperation{}).
			...
			s.L(ctx).Errorw("tr created but write-back failed (orphan TR)",
