...
func TestUpdateTenantOps_SkipDeployDuringImport(t *testing.T) {
	s := setupDRTest(t)
	...
	// Simulate import in progress: set RequestState to READY and ImportState to IN_PROGRESS
	testDB.Model(&db.ArtifactTenantOperation{}).Where("id = ?", opID).Updates(map[string]any{
		"request_state": lifecycle.RequestReady,
		"import_state":  lifecycle.ImportInProgress,
	})

	// SkipDeploy should still be allowed during import
	updateOps := []OpUpdateItem{{SkipDeploy: true}}
	updateOps[0].ID = opID

	_, err = svc.UpdateTenantOps(s.dr.ID, updateOps, "test-user")
	if err != nil {
		t.Fatalf("UpdateTenantOps (SkipDeploy during import) should succeed, got: %v", err)
	}
...
func TestUpdateTenantOps_SkipDeployBlockedDuringDeploy(t *testing.T) {
	s := setupDRTest(t)
	...
	// Simulate deploy in progress
	testDB.Model(&db.ArtifactTenantOperation{}).Where("id = ?", opID).Updates(map[string]any{
		"request_state": lifecycle.RequestReady,
		"import_state":  lifecycle.ImportComplete,
		"deploy_state":  lifecycle.DeployInProgress,
	})

	// SkipDeploy should be blocked when deploy is in progress
	updateOps := []OpUpdateItem{{SkipDeploy: true}}
	updateOps[0].ID = opID

	_, err = svc.UpdateTenantOps(s.dr.ID, updateOps, "test-user")
	if err == nil {
		t.Fatal("UpdateTenantOps (SkipDeploy during active deploy) should fail")
	}
}

func TestUpdateTenantOps_SkipDeployBlockedWhenDeployComplete(t *testing.T) {
	s := setupDRTest(t)
	svc := newTestService(s.cpiFactory)
	...
	testDB.Model(&db.ArtifactTenantOperation{}).Where("id = ?", opID).Updates(map[string]any{
		"request_state": lifecycle.RequestReady,
		"import_state":  lifecycle.ImportComplete,
		"deploy_state":  lifecycle.DeployComplete,
	})
	...
	_, err = svc.UpdateTenantOps(s.dr.ID, updateOps, "test-user")
	if err == nil {
		t.Fatal("UpdateTenantOps (SkipDeploy after deploy complete) should fail")
	}
}

func TestUpdateTenantOps_UnSkipDeployWhenQueued(t *testing.T) {
	s := setupDRTest(t)
	svc := newTestService(s.cpiFactory)
	...
	// Simulate: import done, deploy disabled (skipped), now user wants to un-skip
	testDB.Model(&db.ArtifactTenantOperation{}).Where("id = ?", opID).Updates(map[string]any{
		"request_state": lifecycle.RequestReady,
		"import_state":  lifecycle.ImportComplete,
		"deploy_state":  lifecycle.DeployDisabled,
	})

	updateOps := []OpUpdateItem{{SkipDeploy: false}}
	updateOps[0].ID = opID

	_, err = svc.UpdateTenantOps(s.dr.ID, updateOps, "test-user")
	if err != nil {
		t.Fatalf("UpdateTenantOps (un-skip after import complete) should succeed, got: %v", err)
	}
...
func TestUpdateTenantOps_SkipDeployWhenDeployFailed(t *testing.T) {
	s := setupDRTest(t)
	svc := newTestService(s.cpiFactory)
	...
	// Simulate: deploy failed, user wants to skip instead of retry
	testDB.Model(&db.ArtifactTenantOperation{}).Where("id = ?", opID).Updates(map[string]any{
		"request_state": lifecycle.RequestReady,
		"import_state":  lifecycle.ImportComplete,
		"deploy_state":  lifecycle.DeployFailed,
	})
	...
	_, err = svc.UpdateTenantOps(s.dr.ID, updateOps, "test-user")
	if err != nil {
		t.Fatalf("UpdateTenantOps (skip after deploy failed) should succeed, got: %v", err)
	}
...
func TestUpdateTenantOps_TRChangeBlockedWhenNotPending(t *testing.T) {
	s := setupDRTest(t)
	svc := newTestService(s.cpiFactory)
	...
	// Simulate: TR resolved, request is READY
	testDB.Model(&db.ArtifactTenantOperation{}).Where("id = ?", opID).Updates(map[string]any{
		"transport_request_number": "TR-001",
		"request_state":            lifecycle.RequestReady,
	})

	updateOps := []OpUpdateItem{{TransportRequestNumber: "TR-NEW"}}
	updateOps[0].ID = opID

	_, err = svc.UpdateTenantOps(s.dr.ID, updateOps, "test-user")
	if err == nil {
		t.Fatal("UpdateTenantOps (TR change when READY) should fail")
	}
}

func TestUpdateTenantOps_TRBlockedButSkipDeployAloneAllowed(t *testing.T) {
	s := setupDRTest(t)
	svc := newTestService(s.cpiFactory)
	...
	// Simulate: READY state (TR locked), import in progress
	testDB.Model(&db.ArtifactTenantOperation{}).Where("id = ?", opID).Updates(map[string]any{
		"transport_request_number": "TR-001",
		"request_state":            lifecycle.RequestReady,
		"import_state":             lifecycle.ImportInProgress,
	...
	// Only SkipDeploy change (TR stays the same) — should be allowed
	updateOps := []OpUpdateItem{{TransportRequestNumber: "TR-001", SkipDeploy: true}}
	updateOps[0].ID = opID

	_, err = svc.UpdateTenantOps(s.dr.ID, updateOps, "test-user")
	if err != nil {
		t.Fatalf("UpdateTenantOps (only SkipDeploy, TR unchanged) should succeed, got: %v", err)
	}
	...
	if dbOp.TransportRequestNumber != "TR-001" {
		t.Errorf("expected TR unchanged, got %q", dbOp.TransportRequestNumber)
	}
}
