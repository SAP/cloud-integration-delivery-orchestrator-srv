...
### Phase 3: 后端测试 — ✅ COMPLETE

**范围**：Backend
**状态**：已完成
**变更文件**：
- `service/testhelper_test.go` — 扩展测试基础设施：
  - AutoMigrate 新增 `Artifact`, `DeliveryRequest`, `ArtifactTenantOperation`, `Condition`
  - 新增 `mockTMSClient`（实现 `TransportService` 接口）
  - 新增 `mockNotifier`（no-op `Notifier` 实现）
  - 新增 `mockCPIClientWithDesignTime`（支持 design-time 版本返回，用于 downgrade 测试）
  - `testCleanup` 扩展 `drIDs` + `artifactIDs` 追踪，cleanup 顺序：conditions → ops → DRs → artifacts → snapshots → rules → tenants
  - `newTestService` 改为支持可选 `testServiceOpts`（TMS/Notifier/GetUserEmail），兼容现有测试
  - 新增 seed helpers：`seedDeliveryRequest`, `seedOp`, `seedArtifact`, `validTR`
- `service/dr_test.go` — Phase 1 测试（9 个）：
  - `TestInsertTenantOps_EmptyTR` / `_WithTR` / `_WithInvalidTR`
  - `TestUpdateTenantOps_EmptyToNonEmpty` / `_EmptyToEmpty`
  - `TestRequestApproval_MissingTR` / `_WithValidTR`
  - `TestApprove_MissingTR` / `_AllTRPresent`
- `service/version_compare_test.go` — Phase 2 测试（14 个）：
  - `TestPreviewDR_Basic` / `_DraftDetection` / `_VersionPatternDetection` / `_DuplicateDetection` / `_NoMismatch` / `_NoSnapshot`
  - `TestCreateDR_Basic` / `_SnapshotStale` / `_EmptyArtifactKeys` / `_JiraRequired` / `_SnapshotFKSet` / `_DuplicateIncluded` / `_VersionDowngradeSkip` / `_AutoGeneratedName`

**备注**：
- 所有 23 个新测试通过 `-count=2` 验证幂等性
...
| `TestInsertTenantOps_WithInvalidTR` | 1 | TR 无效时 InsertOps 拒绝 | ✅ |
...
| `TestRequestApproval_WithValidTR` | 1 | 提交审批时有 TR → 通过 | ✅ |
...
| `TestPreviewDR_NoMismatch` | 2 | 无 mismatch → error | ✅ |
| `TestPreviewDR_NoSnapshot` | 2 | 无 snapshot → error | ✅ |
| `TestCreateDR_Basic` | 2 | 选中 artifacts → 创建 DR + ops | ✅ |
| `TestCreateDR_SnapshotStale` | 2 | snapshotCompletedAt 不匹配 → error | ✅ |
| `TestCreateDR_VersionDowngradeSkip` | 2 | 降级检查失败 → 容错，返回 errors | ✅ |
| `TestCreateDR_EmptyArtifactKeys` | 2 | 空 artifactKeys → error | ✅ |
| `TestCreateDR_JiraRequired` | 2 | Rule 要求 JIRA 但未提供 → error；提供后 → 成功 | ✅ |
...
| `TestCreateDR_AutoGeneratedName` | 2 | 空 name → 自动生成 "Auto DR - ..." 格式 | ✅ |
